...
var upload = multer({storage: storage, limits: { fileSize: 20000000, fieldSize: 20000000 }}); // 20mb
...
var hostname = '167.86.101.40';
var port = 8080;

if (process.env.DEVELOPMENT){
	hostname = '167.86.101.40';
	port = 8000;
}
...
// basic auth
function BasicAuth(){
	return basicAuth({ users: GetUsers2(), challenge: true});
...
server.get("/audio", async function(req, res){
...
server.post("/video", upload.single('video'), BasicAuth(), async function(req, res){
...
server.post('/api/register', function(req, res){
	...
	var users = GetUsers2();
	var userExists = users.find(x => x.user == user);
	if (userExists){
		res.end(JSON.stringify("User already exists"));
		return;
	}
	var emailExists = users.find(x => x.user == user);
	if (emailExists){
...
function GetRecordings(user){
	...
	for (var audio of audios){
		if (fs.existsSync(`recordings/${audio.filename}`))
			recordings.push({title: audio.title, description: audio.description, date: audio.date, pageDate: audio.pageDate, url: audio.url, source: audio.filename});
	}
	return recordings;
}
function GetVideos(user){
	...
	for (var video of videos){
		if (fs.existsSync(`videos/${video.filename}`))
			recordings.push({title: video.title, description: video.description, date: video.date, source: video.filename});
	}
	return recordings;
}
...
server.post("/api/upload-audio", BasicAuth(), async function(req, res){
...
server.post('/api/upload', BasicAuth(), function(req, res){
	var body = req.body;
	var user = req.auth.user;
